...
VIDEO = HERE / "actuation_currents_data" / "2026.4.16-decel-3soak-dry-then-rewet.mov"
OUT = HERE / "find_crop_3soak_dry.png"

CROP = dict(y0=725, y1=815, x0=1140, x1=1400)

cap = cv2.VideoCapture(str(VIDEO))
dur = cap.get(cv2.CAP_PROP_FRAME_COUNT) / cap.get(cv2.CAP_PROP_FPS)
fracs = [0.05, 0.2, 0.35, 0.5, 0.65, 0.8, 0.95]

fig, axes = plt.subplots(len(fracs), 1, figsize=(7, 1.1*len(fracs)))
for ax, frac in zip(axes, fracs):
    ...
    ax.axis('off')
plt.suptitle(f"Proposed: y={CROP['y0']}-{CROP['y1']}, x={CROP['x0']}-{CROP['x1']}",
             fontsize=10, y=1.0)
plt.tight_layout()
