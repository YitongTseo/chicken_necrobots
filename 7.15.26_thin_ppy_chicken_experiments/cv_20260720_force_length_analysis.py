"""
CV analysis - 2026-07-20 1000 um decell chicken + PPy, 2.5 mV/s.

Two scope runs, no potentiostat export for this date, so the applied potential is
synthesized from the known protocol rather than read from a PalmSens file:
+1 V -> -1 V -> +1 V at 2.5 mV/s = 800 s per leg, 1600 s per cycle, 3 cycles = 4800 s.

Scope and potentiostat were started together (t0 = 0 for both files, confirmed by
the user); the FORCE log simply kept recording ~479 s after the sweep ended, so
everything past 4800 s is discarded.

  FORCE  run -> CH2, 56.7 mN/V   (CH1 is 0)
  LENGTH run -> CH1, -0.4130 mm/V (CH2 is 0)

Same MAD-outlier / Savitzky-Golay / per-cycle-creep-removal pipeline as
cv_20260715_analysis.ipynb. No stress or strain: sample dimensions not yet measured.
"""
...
V_HI, V_LO = 1.0, -1.0
LEG_S = (V_HI - V_LO) / SCAN_RATE_V_PER_S        # 800 s per sweep leg
PERIOD_S = 2 * LEG_S                              # 1600 s per full cycle
N_CYCLES = 3
TOTAL_S = N_CYCLES * PERIOD_S                     # 4800 s

FORCE_GAIN_MN_PER_V = 56.7        # 1x gain
LENGTH_GAIN_MM_PER_V = -0.4130    # 1x gain (see note in summary re: 10x ambiguity)

# Sample geometry, measured 2026-07-21. Note the "1000um" in the filenames is NOT
# the thickness: at 0.27 mm this slice is thinner than every formalin-fixed sample,
# and its 1.08 mm^2 cross-section is the smallest in the set. L0 is unusually short.
...
RUNS = {
    "FORCE": dict(
        path="07.20.26_1000um_chicken_FORCE_2.5mVpersec_scope_log_20260720_144613.csv",
        channel="ch2_MEAN_V",
        gain=FORCE_GAIN_MN_PER_V,
        unit="mN",
        label="Force",
    ),
    "LENGTH": dict(
...
def applied_potential(t):
    """Triangle sweep: +1 V -> -1 V over the first leg, back up over the second."""
...
def load(run):
    d = pd.read_csv(run["path"])
    d = d.rename(columns={"elapsed_s": "t"})
    n_raw = len(d)
    ...
    # sweep direction: reduction on the way down to -1 V, oxidation on the way back
...
def plot_run(tag, run, d):
    ...
    # --- top: raw time series with the synthesized potential overlaid ----------
    ax = fig.add_subplot(gs[0, :])
    ax.plot(d["t"], d["signal"], lw=0.5, color="0.7", label=f"raw {lab.lower()}")
    ...
    # --- bottom: one CV loop per cycle ---------------------------------------
    for c in range(N_CYCLES):
        axc = fig.add_subplot(gs[1, c])
        s = d[d["cycle"] == c].sort_values("t")
        if len(s) < 2:
            continue
        d_open = s["smooth"].values - s["smooth"].values[0]
        d_closed = s["smooth_closed"].values - s["smooth_closed"].values[0]
        swing = d_closed.max() - d_closed.min()

        axc.plot(s["v_applied"], d_open, color=PALETTE[c], lw=1.1, ls=":",
                 alpha=0.55, label="original (open)")
        axc.plot(s["v_applied"], d_closed, color=PALETTE[c], lw=2.0,
                 label="creep-corrected")
        axc.scatter([s["v_applied"].values[0], s["v_applied"].values[-1]],
                    [d_closed[0], d_closed[-1]], color=PALETTE[c], s=45,
                    ec="k", lw=0.5, zorder=5)
        axc.axhline(0, color="k", lw=0.5, alpha=0.5)
        axc.invert_xaxis()  # sweep runs +1 V -> -1 V, so read left-to-right
        axc.set_xlabel("Applied potential (V)")
        if c == 0:
            axc.set_ylabel(f"delta {lab.lower()} from +1 V start ({unit})")
        axc.grid(alpha=0.3)
        axc.legend(fontsize=7.5, loc="best")
        axc.set_title(f"cycle {c+1}   (swing {swing:.3g} {unit})", fontsize=10)

    fig.suptitle(f"2026-07-20 1000 um decell chicken + PPy - {lab} CV, 2.5 mV/s "
                 f"(+1 V -> -1 V -> +1 V, 3 cycles)", fontweight="bold")
    out = f"cv_20260720_{tag.lower()}_cv.png"
    fig.savefig(out, bbox_inches="tight", dpi=130)
    print(f"  saved {out}")


def overlay(tag, run, d):
    """All three loops on one axes, to show cycle-to-cycle reproducibility."""
    unit, lab = run["unit"], run["label"]
    ...
    ax.set_ylabel(f"delta {lab.lower()} from +1 V start ({unit})")
    ax.grid(alpha=0.3)
    ax.legend(fontsize=9)
    ax.set_title(f"{lab} CV - all cycles overlaid\n(creep-corrected, 2.5 mV/s)",
                 fontsize=11, fontweight="bold")
    out = f"cv_20260720_{tag.lower()}_overlay.png"
    fig.savefig(out, bbox_inches="tight", dpi=130)
    print(f"  saved {out}")
...
for tag, run in RUNS.items():
    ...
    for c in range(N_CYCLES):
        ...
        y = s["smooth_closed"].values - s["smooth_closed"].values[0]
        swing = y.max() - y.min()
        # normalise: force -> stress (MPa), displacement -> strain (%)
        norm = (swing / AREA_MM2 / 1000.0) if tag == "FORCE" else (swing / L0_MM * 100.0)
        summary.append(dict(run=tag, cycle=c + 1, unit=run["unit"],
                            swing=swing,
                            normalised=norm,
                            norm_unit="MPa" if tag == "FORCE" else "%",
                            v_at_max=s["v_applied"].values[int(np.argmax(y))],
                            v_at_min=s["v_applied"].values[int(np.argmin(y))]))

print(f"\n=== geometry: L0={L0_MM} mm, W={WIDTH_MM} mm, T={THICKNESS_MM} mm, "
      f"area={AREA_MM2:.3f} mm^2 ===")
